...
export async function processUserSubscription(db, now, user) {
    ...
    let minutesSinceLastMessage = user.last_contacted === null ? null : Interval.fromDateTimes(user.last_contacted, now).toDuration().as('minutes');
    console.log(user.psid + ": " + minutesSinceLastMessage + " since last message");
    if(minutesSinceLastMessage !== null && minutesSinceLastMessage < MIN_MINUTES_SINCE_LAST_MESSAGE) {
        console.log(user.psid + ": skipping");
        return;
    }
